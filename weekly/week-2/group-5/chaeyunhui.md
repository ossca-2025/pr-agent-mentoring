...
## 2. MCP (Multi-modal Conversational Pretraining)

### 2.1 개념 정의
- MCP란 무엇인가? (다중 모달 대화형 사전 학습)
    - 대형 언어 모델(LLM)이 외부 데이터 및 도구와 상호작용할 수 있도록 해주는 표준화된 프로토콜
    - 기존에는 AI가 사전에 학습된 정보에만 의존해야 했지만, MCP를 사용하면 실시간으로 최신 데이터를 가져와 활용할 수 있음
...
1. 초기 연결 설정
    - 호스트 애플리케이션 실행 시 MCP 클라이언트는 서버에 초기화 요청을 전송
    - 이 과정에서 클라이언트 버전 정보와 지원 기능을 교환하며 상호 호환성을 확인
...
- 프로토콜(Protocol)
    - MCP의 핵심인 통신 규약입니다. MCP는 통신에 JSON-RPC 2.0을 기반으로 한 표준 메시지 형식을 사용
...
1. 보안 문제 :  인증 미정의, 로컬 실행 위험성, 입력 신뢰에 대한 취약점 등이 주요 이슈가 있음
    - ex. 초판 MCP 사양에서는 인증 방식을 포함하지 않았으며, 이로 인해 각 MCP 서버는 자체 방식으로 인증을 처리해야 했고, 어떤 서버는 아예 인증 없이 민감한 데이터에 접근 가능
...
2. UI/UX 한계 : MCP는 LLM이 이해하기 쉬운 인터페이스를 갖고 있지만, 사람이 사용하기에는 불편하거나 위험한 설계 요소들이 존재
...
4. 결과 반환
    - 작업 완료 후 결과물(아티팩트)을 클라이언트 에이전트에 반환
    - 다양한 형식(텍스트, 이미지, 파일 등)으로 결과 전달 가능
...
```
2. 클라이언트와 서버
    - A2A는 클라이언트-서버 구조 기반으로 설계됨
3. 작업(Task)
    - A2A에서의 근본적인 작업 단위
    - 고유한 ID와 상태(submitted, working, input-required, completed 등)를 가짐
    - 모든 에이전트 간 상호작용은 이 작업 수행을 중심으로 이루어짐
4. 메시지와 파트
    - 메시지: 클라이언트와 에이전트 간 통신의 단일 턴
    - 파트: 메시지 내의 콘텐츠 조각으로 텍스트, 파일, 구조화된 데이터 등 다양한 형식 가능
...
### 4.2 현재 연구 동향
- MCP의 빠른 채택(2025년 2월 기준 1,000개 이상 서버 개발)과 A2A의 등장(2025년 4월 구글 발표, 50개 이상 기업 지원)을 보여줌
...
- 기술적 융합: A2A, MCP, RAG와 같은 기술이 하나의 통합 프레임워크로 발전할 가능성도 있. 
